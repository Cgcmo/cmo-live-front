...
export default function Homepage() {
  ...
  const fetchAlbums = async (page = 1) => {
    try {
      const res = await fetch(`${API_URL}/albums?page=${page}&limit=${imagesPerPage}`);
  ...
  const fetchPhotos = async (album, page = 1) => {
    try {
      setIsLoading(true);
      const res = await fetch(`${API_URL}/photos/${album._id}?page=${page}&limit=${imagesPerPage}`);
      const data = await res.json();
  ...
  const handleDownloadAlbum = async (album) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/photos/${album._id}?limit=0`);
      const data = await res.json();
